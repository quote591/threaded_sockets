...
bool Encryption::CreateDHKeys(void)
{
    ...
    Log::s_GetInstance()->m_LogWrite("Encryption::CreateDHKeys", "Created DH keys.");
...
bool Encryption::s_GenerateIV(unsigned char *ivOut, int bytes)
{
    try
    {
    ...
    {
        Log::s_GetInstance()->m_LogWrite("Encryption::s_GenerateIV", "Error: ", err);
        return false;
    }
    return true;
}
...
Encryption::~Encryption()
{
    ...
    Log::s_GetInstance()->m_LogWrite("Encryption::~Encryption", "Freed resources.");
}
