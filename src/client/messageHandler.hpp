...
// Struct to hold the message and its type, needed for queue
struct NetworkMessage
{
    NetworkMessage(unsigned char messageTypeIn, std::string messageIn) : 
                   messageType(messageTypeIn), message(messageIn) {}
...
private:
    ...
    std::queue<std::unique_ptr<NetworkMessage>> m_sendMessageQueue;
...
public:
    ...
    /// @return return unique ptr
    std::unique_ptr<NetworkMessage> m_GetMessageFromSendQueue(void);
